...
class OptimizedDownloader:
    ...
    def download_with_retry_urllib(self, url: str, data: Optional[bytes] = None,
                                  max_retries: int = 3, timeout: Optional[int] = None) -> Optional[bytes]:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                url_request = urllib.request.Request(url, data=data, headers={'User-Agent': current_ua})
                ...
                if json_content and len(json_content) > 10:
                    ...
                    logging.warning('  Empty/small response on attempt %d: %d bytes',
                                  attempt + 1, len(json_content) if json_content else 0)
                    self.consecutive_failures += 1
    ...
    def download_with_retry(self, url: str, method: str = 'GET', data: Optional[str] = None,
                           max_retries: int = 3, timeout: Optional[int] = None) -> Optional[bytes]:
