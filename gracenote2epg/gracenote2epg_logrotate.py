...
class CopyTruncateTimedRotatingFileHandler(logging.handlers.BaseRotatingHandler):
    ...
    def __init__(self, filename: str, when: str = 'midnight', interval: int = 1,
                 backup_count: int = 7, encoding: Optional[str] = None):
        """
        ...
        logging.debug('Next log rotation scheduled for: %s', next_rotation.strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def _compute_next_rollover(self) -> float:
        ...
        if self.when == 'MIDNIGHT' or self.when == 'DAILY':
            ...
            next_rollover = current_time.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
            ...
            next_rollover = current_time.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=days_until_sunday)
            return next_rollover.timestamp()
            ...
            if current_time.month == 12:
                next_rollover = current_time.replace(year=current_time.year + 1, month=1, day=1,
                                                   hour=0, minute=0, second=0, microsecond=0)
            else:
                next_rollover = current_time.replace(month=current_time.month + 1, day=1,
                                                   hour=0, minute=0, second=0, microsecond=0)
            return next_rollover.timestamp()
    ...
    def _analyze_log_periods(self, log_file: Path) -> Dict[str, Dict]:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if timestamp_match:
                    try:
                        ...
                        period_start, period_end, period_suffix = self._get_period_info(entry_datetime)
                        ...
                        if period_suffix not in periods_data:
                            ...
                            is_complete = self._is_period_complete(period_start, period_end, current_datetime)
                            ...
                            logging.debug('Found %s %s (%s to %s) - Complete: %s',
                                        self.period_name, period_suffix,
            ...
            logging.info('  Complete %ss found: %d (%s)',
                        self.period_name, len(complete_periods), ', '.join(complete_periods))
            logging.info('  Current %s: %s (%s)',
                        self.period_name, ', '.join(current_periods) if current_periods else 'none',
    ...
    def _is_period_complete(self, period_start: datetime, period_end: datetime, current_datetime: datetime) -> bool:
        ...
        if self.when == 'MIDNIGHT' or self.when == 'DAILY':
            ...
            return (period_start.year, period_start.month) < (current_datetime.year, current_datetime.month)
    ...
    def _get_week_start(self, dt: datetime) -> datetime:
        ...
        week_start = dt.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=days_since_sunday)
        return week_start
    ...
    def _perform_multi_period_rotation(self, periods_data: Dict[str, Dict]):
        ...
        try:
            ...
            periods_to_rotate = {period: data for period, data in periods_data.items() if data['complete']}
            current_period_data = {period: data for period, data in periods_data.items() if not data['complete']}

            if not periods_to_rotate:
                logging.debug('No complete %ss found - no rotation needed', self.period_name)
                return

            logging.info('Multi-%s rotation starting: %d complete %ss to rotate',
                        self.period_name, len(periods_to_rotate), self.period_name)
            ...
            try:
                ...
                if current_lines:
                    logging.info('Current log rebuilt with %d lines from current %s',
                               len(current_lines), self.period_name)
                else:
                    logging.info('Current log truncated - no current %s data found', self.period_name)
            ...
            self.rollover_at = self._compute_next_rollover()
            next_rotation = datetime.fromtimestamp(self.rollover_at)
            logging.debug('Next rotation rescheduled for: %s', next_rotation.strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def doRollover(self):
        ...
        try:
            ...
            if Path(self.baseFilename).exists():
                ...
                logging.info('Log rotated: %s -> %s',
                           Path(self.baseFilename).name, Path(backup_filename).name)
        ...
        try:
            ...
            for file_path, _ in files_to_remove:
                try:
                    ...
                    logging.warning('Could not remove old log backup %s: %s', file_path.name, str(e))
